...
TEXT_PROMPT = "hand." 
data_type = args.type
IMG_PATH = "Path to /data/"+data_type+"/images/"
...
OUTPUT_DIR = "Path to clip_ego/"+data_type+"/"
...
for img_path in os.listdir(folder_path):
    ...
    # clip2_dir="./clip_ego/"+data_type+"/"
    # clip2_dir_=Path(clip2_dir)
    # clip2_dir_.mkdir(parents=True, exist_ok=True)

    # if len(input_boxes)>0: 
    #     cv2.imwrite(os.path.join(clip2_dir, filename), annotated_frame)    
    # else:
    #     cv2.imwrite(os.path.join(clip2_dir, filename), img)    
