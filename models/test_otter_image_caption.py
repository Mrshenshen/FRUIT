...
def prompt_template_planning(prompt,caption):
    return f"You are a person in the situation shown in the image. Answer your question in a detailed and helpful way.\n Here is the caption of the image:{caption} \nQuestion: {prompt} "

def prompt_template(prompt,caption):
    return f"You are a person in the situation shown in the image. Answer the following question shortly and accurately! Keep your answer as short as possible! \n Here is the caption of the image:{caption} \nQuestion: {prompt} "
...
class TestOtterImage:
    def __init__(self, name='image', device=None) -> None:
        if name == 'image':
            model_path = r'your path to OTTER-Image-MPT7B'
        elif name == 'video':
            model_path = r'your path to OTTER-Video-LLaMA7B-DenseCaption'
    ...
    @torch.no_grad()
    def generate(self, raw_image, question,caption, max_new_tokens=30, planning=False):
